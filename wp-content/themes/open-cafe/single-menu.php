<?php get_header(); ?>

<main class="menu-lists">
    <ul class="breadcrumbs">
        <li><a href="<?php echo esc_url(home_url()); ?>">HOME</a></li>
        <span>></span>
        <?php
        // 「メニュー」ラベルを取得
        $obj = get_post_type_object('menu');
        $label = 'メニュー';
        if ( $obj && isset($obj->labels->name)) {
            $label = $obj->labels->name;
        }
        ?>
        <li>
            <a href="<?php echo esc_url(get_post_type_archive_link('menu')); ?>">
                <?php echo post_type_archive_title(); ?>
            </a>
        </li>
        <span>></span>
        <li>パスタ</li>
    </ul>
    <div class="container">
        <section class="menu__contents">
            <div class="tab-buttons">
                <div class="tab-buttons__inner">
                    <div class="tab-button btn-active" data-id="pasta">
                        <a href="#pasta" class="btn-text">パスタ</a>
                    </div>
                    <div class="tab-button" data-id="salad">
                        <a href="#salad" class="btn-text">サラダ</a>
                    </div>
                    <div class="tab-button" data-id="sweets">
                        <a href="#sweets" class="btn-text">パン&<br />スイーツ</a>
                    </div>
                    <div class="tab-button" data-id="drink">
                        <a href="#drink" class="btn-text">ドリンク</a>
                    </div>
                </div>
            </div>
            <div class="menu__images">
                <div id="pasta" class="menu__images-contents menu-active">
                    <div class="menu__images-inner">
                        <div class="menu__image">
                            <img src="../img/menu/pasta1.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">テキストテキストテキストの○○○○風パスタ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/pasta2.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">テキストテキストテキストの○○○○風パスタ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/pasta3.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">テキストテキストテキストの○○○○風パスタ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/pasta4.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">テキストテキストテキストの○○○○風パスタ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/pasta5.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">テキストテキストテキストの○○○○風パスタ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/pasta1.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">テキストテキストテキストの○○○○風パスタ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/pasta2.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">テキストテキストテキストの○○○○風パスタ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/pasta3.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">テキストテキストテキストの○○○○風パスタ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/pasta4.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">テキストテキストテキストの○○○○風パスタ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/pasta5.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">テキストテキストテキストの○○○○風パスタ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/pasta1.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">テキストテキストテキストの○○○○風パスタ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/pasta2.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">テキストテキストテキストの○○○○風パスタ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/pasta3.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">テキストテキストテキストの○○○○風パスタ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/pasta4.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">テキストテキストテキストの○○○○風パスタ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                    </div>
                </div>
                <div id="salad" class="menu__images-contents">
                    <div class="menu__images-inner">
                        <div class="menu__image">
                            <img src="../img/menu/salad1.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○風サラダ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/salad2.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○風サラダ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/salad3.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○風サラダ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/salad4.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○風サラダ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/salad5.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○風サラダ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/salad1.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○風サラダ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/salad2.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○風サラダ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/salad3.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○風サラダ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/salad4.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○風サラダ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/salad5.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○風サラダ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/salad1.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○風サラダ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/salad2.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○風サラダ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/salad3.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○風サラダ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/salad4.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○風サラダ</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                    </div>
                </div>
                <div id="sweets" class="menu__images-contents">
                    <div class="menu__images-inner">
                        <div class="menu__image">
                            <img src="../img/menu/sweets1.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○サンド</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/sweets2.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○サンド</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/sweets3.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○サンド</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/sweets4.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○サンド</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/sweets5.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○サンド</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/sweets1.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○サンド</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/sweets2.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○サンド</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/sweets3.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○サンド</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/sweets4.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○サンド</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/sweets5.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○サンド</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/sweets1.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○サンド</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/sweets2.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○サンド</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/sweets3.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○サンド</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/sweets4.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○サンド</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                    </div>
                </div>
                <div id="drink" class="menu__images-contents">
                    <div class="menu__images-inner">
                        <div class="menu__image">
                            <img src="../img/menu/drink1.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○ドリンク</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/drink2.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○ドリンク</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/drink3.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○ドリンク</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/drink4.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○ドリンク</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/drink5.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○ドリンク</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/drink1.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○ドリンク</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/drink2.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○ドリンク</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/drink3.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○ドリンク</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/drink4.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○ドリンク</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/drink5.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○ドリンク</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/drink1.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○ドリンク</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/drink2.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○ドリンク</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/drink3.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○ドリンク</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                        <div class="menu__image">
                            <img src="../img/menu/drink4.jpg" alt="パスタ画像">
                            <div class="menu__image-text">
                                <div class="desc">○○○○ドリンク</div>
                                <div class="price">780 yen</div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
    </div>
    <?php get_template_part('template-parts/access'); ?>
</main>
<?php get_footer(); ?>
